...
| `essai_statistiques.py` | L'écran des statistiques : une partie de sudoku menée jusqu'au bout affiche « 1 partie » (au singulier) et un record cohérent, un jeu jamais ouvert dit « Pas encore joué » ; « Sauvegarder mes scores » produit un vrai téléchargement, lu comme un fichier JSON ; une seconde partie change les chiffres, et restaurer le fichier téléchargé les ramène exactement à l'état sauvegardé **et** affiche « Sauvegarde restaurée. » ; un fichier qui n'est pas une sauvegarde produit un message d'erreur clair, sans rien casser derrière. |
